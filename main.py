...
if __name__ == '__main__':
    ...
    parser.add_argument('--cml_weight', default=10.0, type=float, help='weight of context moment learning loss')
    ...
    model = DSMN_SAIM(len(word2id), args.embedding_dim, embedding_matrix, args.hidden_size, args.n_hop).cuda()
    optimizer = optim.Adam(model.parameters(), lr=args.learning_rate, weight_decay=args.l2_reg)
    best_acc, best_f, best_epoch, es_cnt = 0.0, 0.0, 0, 0
    best_model = DSMN_SAIM(len(word2id), args.embedding_dim, embedding_matrix, args.hidden_size, args.n_hop).cuda()
    best_y_pred = []
    ...
    for i in range(args.n_epoch):
        ...
        for sample, _ in get_batch_data(train_data, args.batch_size, True):
            ...
            optimizer.zero_grad()
            tloss = 1.0 * loss + args.cml_weight * sloss
            tloss.backward()
            optimizer.step()
            cost += tloss.item() * num
            cnt += num
        ...
        for sample, _ in get_batch_data(test_data, args.batch_size, False):
            loss, sloss, num, correct_num, y_pred, y_gold = model.forward(sample, 0)
            tloss = 1.0 * loss + args.cml_weight * sloss
            cost += tloss.item() * num
            cnt += num
            total_correct_num += correct_num.item()
        ...
        print(
            'Epoch %d, Train Loss %.5f, Train Acc %.5f, Train F1 %.5f, Test Loss %.5f, Test Acc %.5f, Test F1 %.5f' % (
                i, train_loss, train_acc, train_f, test_loss, test_acc,
                test_f))
