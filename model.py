...
class DSMN_SAIM(nn.Module):
    ...
    def __init__(self, embedding_size, embedding_dimension, embedding_matrix, hidden_size, n_hop):
        ...
        # self.attention2 = nn.Linear(self.hidden_size * 2, self.hidden_size * 2).cuda()
    ...
    def forward(self, data, dropout=0.0):
        ...
        for h in range(self.n_hop):
            ...
            memory = specific_sentence_outputs * sentences_info.unsqueeze(-1).expand(total_num, max_sentence_len,
                                                                                     self.hidden_size * 2)
            # window
            # window_memory = self.avepool[h](memory)
            # attention_score = torch.bmm(self.attention1(window_memory), aspect_output.unsqueeze(-1))
        ...
        for i in range(len(num)):
            ...
            # cur_var_vec = var_vec[cnt:cnt + cur_num, :]
            cur_aspect_loc = aspect_locs[i, :cur_num, :cur_num]
            ...
            # method 2: attention
            # score = torch.bmm(self.aspect_attention(weighted_cur_e),
            #                   torch.cat([cur_e, cur_var_vec], dim=-1).unsqueeze(-1))
            # score = torch.bmm(self.aspect_attention(weighted_cur_e), cur_var_vec.unsqueeze(-1))
            # score = score - torch.eye(cur_num).unsqueeze(-1).cuda() * 1e12
            # glo_vec = torch.sum(
            #     F.softmax(score, dim=-1).expand(cur_num, cur_num, self.hidden_size * 2) * weighted_cur_e, dim=1)
