...
class DSMN_SAIM(nn.Module):
    ...
    def forward(self, data, dropout=0.0):
        ...
        for i in range(len(sentences)):
            ...
            specific_aspect_vars.append((aspect_vars_pos[i, :num[i], :] + aspect_vars_sem[i, :num[i], :]) / 2.0)
            specific_sentence_infos.append((sentence_infos_pos[i, :num[i], :] + sentence_infos_sem[i, :num[i], :, :]) / 2.0)
            specific_sentence_locs.append((sentence_locs_pos[i, :num[i], :max_sentence_len] + sentence_locs_sem[i, :num[i], :max_sentence_len]) / 2.0)
        ...
        for i in range(len(num)):
            ...
            cur_aspect_loc = (aspect_locs_pos[i, :cur_num, :cur_num] + aspect_locs_sem[i, :cur_num, :cur_num]) / 2.0
