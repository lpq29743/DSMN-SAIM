...
class DSMN_SAIM(nn.Module):
    ...
    def __init__(self, embedding_size, embedding_dimension, embedding_matrix, hidden_size, n_hop):
        ...
        self.dist_sen_att = nn.Linear(self.hidden_size * 2, 1).cuda()
        self.select_w = nn.Linear(self.hidden_size * 2, 1)
    ...
    def forward(self, data, dropout=0.0):
        ...
        sentence_num = sentence_outputs.size()[0]
        max_sentence_len = sentence_outputs.size()[1]
        sentence_mask = torch.ones(sentence_num, max_sentence_len).cuda()
        for i in range(sentence_num):
            sentence_mask[i, sentence_lens[i]:] = 0
        
        sentence_outputs_flatten = sentence_outputs.view(-1, self.hidden_size * 2)
        sentence_outputs_weight = self.dist_sen_att(sentence_outputs_flatten)
        sentence_outputs_weight = sentence_outputs_weight.view(-1, max_sentence_len)
        sentence_outputs_weight = sentence_outputs_weight - (1 - sentence_mask) * 1e12
        sentence_outputs_weight = F.softmax(sentence_outputs_weight, dim=1).unsqueeze(-1).expand(sentence_num,
                                                                                                 max_sentence_len,
                                                                                                 self.hidden_size * 2)
        weighted_sentence_outputs = sentence_outputs_weight * sentence_outputs
        ...
        w = self.sigmoid(self.select_w(sentence_output))
        ...
        for i in range(len(sentences)):
            ...
            specific_aspect_vars.append(w[i] * aspect_vars_pos[i, :num[i], :] + (1 - w[i]) * aspect_vars_sem[i, :num[i], :])
            specific_sentence_infos.append(w[i] * sentence_infos_pos[i, :num[i], :] + (1 - w[i]) * sentence_infos_sem[i, :num[i], :, :])
            specific_sentence_locs.append(w[i] * sentence_locs_pos[i, :num[i], :max_sentence_len] + (1 - w[i]) * sentence_locs_sem[i, :num[i], :max_sentence_len])
        ...
        for h in range(self.n_hop):
            ...
            attention_score = attention_score.squeeze(-1) - (1 - torch.ceil(sentences_info)) * 1e12
        ...
        for i in range(len(num)):
            ...
            cur_aspect_loc = w[i] * aspect_locs_pos[i, :cur_num, :cur_num] + (1 - w[i]) * aspect_locs_sem[i, :cur_num, :cur_num]
